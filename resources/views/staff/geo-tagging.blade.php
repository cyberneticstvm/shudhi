...
<div class="p-4">
    <form method="post" action="{{ route('staff.geo.tagging.save') }}">
        ...
        <div class="mb-4">
            ...
            <div class="input-group input-group-lg bg-white shadow-sm rounded overflow-hiddem">
                <span class="input-group-text bg-white"><i class="bi bi-file text-muted"></i></span>
                {{ html()->select('district_id', $districts->pluck('name', 'id'), old('district_id'))->class('form-control select2 selChange')->attribute('data-container', 'lbody')->attribute('data-type', 'di')->placeholder('Select') }}
            </div>
            @error('district_id')
        ...
        <div class="mb-4">
            ...
            <div class="input-group input-group-lg bg-white shadow-sm rounded overflow-hiddem">
                ...
                {{ html()->select('localbody_id', '', old('localbody_id'))->class('form-control lbody selChange')->attribute('data-type', 'lb')->attribute('data-container', 'ward')->placeholder('Select') }}
            </div>
            @error('localbody_id')
        ...
        <div class="mb-4">
            ...
            <div class="input-group input-group-lg bg-white shadow-sm rounded overflow-hiddem">
                <span class="input-group-text bg-white"><i class="bi bi-file text-muted"></i></span>
                {{ html()->select('ward_id', '', old('ward_id'))->class('form-control ward')->placeholder('Select') }}
            </div>
            @error('ward_id')
